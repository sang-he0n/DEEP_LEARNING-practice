...
def load_dataset(batch_size:int, mode:'str') -> tuple :
    ...
    if mode == "train" :
        img_tf_train = torchvision.transforms.Compose(
            transforms=[
                ...
                torchvision.transforms.ToTensor(),
                torchvision.transforms.Normalize(mean=(0.4914, 0.4822, 0.4465), std=(0.2023, 0.1994, 0.2010)) # WHY?
            ]
        )
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train, batch_size=batch_size, num_workers=0)
